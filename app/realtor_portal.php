...
// Allow realtor and admin into this portal
...
$myListings = array_values(array_filter($listingsAll, fn($L) => strtolower((string)($L['agent_email'] ?? '')) === strtolower($meEmail)));
...
// Fallback display name if session name not set
if ($meName === '') {
    $meName = $meEmail !== '' ? preg_replace('/@.*$/', '', $meEmail) : 'Realtor';
}

// Small helpers for icons
function starBadge(string $type = 'gold'): string {
...
function avatarDot(string $seed): string {
...
<style>
  :root{
    ...
    --edge:#345; --trim:#8ec3c7; --accent:#d4af37;
  ...
  *{ box-sizing: border-box; }
  ...
  .bar-actions{ display:flex; gap:12px; flex-wrap:wrap; }
  .btn{
  ...
  .pill{
    ...
    padding:10px 12px; margin:10px 6px; text-decoration:none; color:inherit;
  }
  ...
  .rail{ width:10px; min-width:10px; background:var(--rail); border-radius:10px; height:24px; }
      ...
      <div class="list">
        ...
        <?php else: foreach ($recentViews as $row): ?>
          <a class="pill" href="/app/timeline.php?case=<?=h($row['case_id'] ?? '')?>">
            <div class="left">
              <?=starBadge('gold') . avatarDot((string)(($row['client_name'] ?? '') . ($row['case_id'] ?? '')))?>
    ...
    <section class="card">
      ...
      <div class="list">
        ...
        <?php else: foreach ($contacts as $C): ?>
          <div class="pill">
            <div class="left">
              <?=starBadge('gold')?><?=avatarDot((string)($C['email'] ?? $C['name'] ?? ''))?>
              <span class="label"><?=h($C['name'] ?? 'Contact')?><?= isset($C['role']) ? ' — ' . h(ucfirst((string)$C['role'])) : '' ?></span>
            </div>
